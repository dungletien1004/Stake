...
const DungContract = new web3.eth.Contract(DungToken.abi);
// var Web3 = artifacts.require("../src/js/web3.min.js");
contract('Staking', async function(accounts){
  ...
  it("staking", async () => {
    const LTDToken = await DungToken.deployed({form: accounts[1]});
    ...
    await stakeToken.approve(staker.address, depositAmount, defaultOptions);
    await staker.deposit(depositAmount, apy, defaultOptions);
    ...
    const contract = await staker.getStakerFromId(accounts[0], 0);
    console.log('contract', contract);
    const rewardNow = await staker.getRewardOfStakerWithId(accounts[0], 0);
    console.log('reward', Number(rewardNow));
    const withdraw = await staker.withdraw(0,defaultOptions);
